...
const paintGame = () => {
  ...
  html += `<div class="flex-grow-1">${getPaintGameBoard(
...
const getPaintWinnerPlayer = (player) => {
  let html = `
    <div class="d-flex">
      ...
      <div class="text-border">${player.name.toUpperCase()} WIN !!!</div>
      <div class="crown-container"><svg viewBox="0 0 50 30"><use href="#crown" x="0" y="-10" /></svg></div>
    </div>`;
  return html;
};
...
const cpuPlay = (gameBoard, player) => {
  ...
  if (positionsForWin.length > 0 && Math.floor(Math.random() * 100) > 20) {
    ...
    if (playsForWin.length > 0) {
      const objectIndex = Math.floor(Math.random() * (playsForWin.length - 1));
  ...
  if (
    ...
    if (playsForLose.length > 0) {
      const objectIndex = Math.floor(Math.random() * (playsForLose.length - 1));
...
const getPositionsThatCompleteLinesByDirection = (
  ...
  startPositions.forEach((startPosition) => {
    const directionGameBoardCells = getGameBoardsCellByPosition(startPosition);
    const positions = getPositionsThatCompleteLines(directionGameBoardCells);
    positionsForCompleteLines = positionsForCompleteLines.concat(positions);
  });
  return positionsForCompleteLines;
};
...
const getPositionsThatCompleteLines = (directionGameBoardCells) => {
  ...
  for (let index = 0; index < directionGameBoardCells.length - 3; index++) {
    ...
    if (
      range.filter((gameBoardCell) => gameBoardCell.player === null).length !==
      1
    ) {
      continue;
    }
    ...
    if (
      range.some(
        (gameBoardCell) =>
          gameBoardCell.player && gameBoardCell.player.id !== playerNotNull.id
      )
    ) {
      continue;
    }
...
class PlayerManger {
  ...
  setPlayerType = (playerId, playerType) => {
    ...
    if(this.players.every(player => player.type === PLAYERS_TYPES.CPU)){
      this.players.find(player => player.id !== playerId).type = PLAYERS_TYPES.HUMAN
    }
  };
...
class GameBoard {
  ...
  getFirstFreeGameBoardCellInColumns = () => {
    ...
    for (let column = 0; column < this.columnsCount; column++) {
      const gameBoardCell = this.getFirstFreeGameBoardCellInColumn(column);
      if (gameBoardCell) {
  ...
  getFirstFreeGameBoardCellInColumn = (columnIndex) => {
    let foundGameBoardCell = this.board.find(
      (gameBoardCell) =>
        gameBoardCell.column === columnIndex && gameBoardCell.player === null
    );
  ...
  insertPlayerTokenByPosition = (columnIndex, rowIndex, player) => {
    let index = this.board.findIndex(
      (gameBoardCell) =>
        gameBoardCell.column === columnIndex && gameBoardCell.row === rowIndex
    );
    if (index !== -1) {
  ...
  checkCompleteLinesByPosition = (columnIndex, rowIndex, player) => {
    ...
    const directionsFunctions = [
      ...
      () =>
        this.getUpwardDiagonalGameBoardsCellsByPosition(columnIndex, rowIndex),
      () =>
    ...
    directionsFunctions.forEach((directionFunction) => {
      ...
      const gameBoardsCellWithLine = this.searchLineInGameBoardDirectionCells(
    ...
    if (gameBoardCellsInLines.length > 0) {
      this.board = this.checkAsLineCompleteGameObjects(gameBoardCellsInLines);
    }
  };
  ...
  getHorizontalGameBoardsCellsByPosition = (rowIndex) => {
    return this.board.filter((gameBoardCell) => gameBoardCell.row === rowIndex);
  };
  ...
  getUpwardDiagonalGameBoardsCellsByPosition = (columnIndex, rowIndex) => {
    return this.board.filter(
      (gameBoardCell) =>
        gameBoardCell.column - gameBoardCell.row === columnIndex - rowIndex
    );
  };
